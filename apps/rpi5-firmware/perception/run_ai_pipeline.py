...
def _build_left_panel(
	...
	if bev is not None:
		...
		cv2.line(out, (wb, hb), (wb2, hb), _C_ROI, 3) # Bottom Line
		cv2.line(out, (wu, hu), (wu2, hu), _C_ROI, 3) # Up Line
		cv2.line(out, (wu, hu), (wb, hb), _C_ROI, 3) # Left Line
		cv2.line(out, (wu2, hu), (wb2, hb), _C_ROI, 3) # Right Line
	...
	cv2.putText(out, "ANGLE " + str(debug.angle), (180, 27),
				cv2.FONT_HERSHEY_SIMPLEX, 0.62, _C_LABEL, 1)
	cv2.putText(out, "CTE " + str(round(debug.cte, 2)), (320, 27),
				cv2.FONT_HERSHEY_SIMPLEX, 0.62, _C_LABEL, 1)
...
def _build_right_panel(
	...
	bev: "Bev | None",
	virtual_side=None,
	left_det_pts: "list | None" = None,
	right_det_pts: "list | None" = None,
) -> np.ndarray:
	...
	# Sliding window detection centroids — only windows that found real pixels
	for det_list, fill, outline in (
		(left_det_pts or [],  _C_LEFT_DIM,  _C_LEFT),
		(right_det_pts or [], _C_RIGHT_DIM, _C_RIGHT),
	):
		for x, y in det_list:
...
if __name__ == '__main__':
	...
	try:
		...
		if args.enable_lka:
			#  (sx, sy, w, h)
			lka_bev = Bev(fov=200, roi=(0, 0, CAM_WIDTH, CAM_HEIGHT), trapz=trapz, margin=0)
			lka_sw = SlidingWindow(MODEL_WIDTH)
			lka_vl = VirtualLane()

		infer_engine = Inference(camera, MODEL_PATH)
		post_processor = PostProcessor(input_size=(MODEL_HEIGHT, MODEL_WIDTH), strides=(8, 16, 32), model_name=MODEL_FAMILY, temporal_alpha=1)
		for frame, infer_results in infer_engine.run_inference():
			...
			if lka_bev is not None and lane_mask is not None:
				...
				left_pts, right_pts, left_det_pts, right_det_pts = lka_sw.get_lane_points(bev_frame, n_points=12)
				# Fit to parametric model
				# print(left_pts, right_pts)
				...
				# print("After Coeffs", left_coeffs, right_coeffs)
				left_coeffs, right_coeffs = lka_sw.validate_lanes(
					left_coeffs, right_coeffs, oh, ow
				)
				# print("After Validate", left_coeffs)
				left_coeffs, right_coeffs, virtual_side = lka_vl.update(
					left_coeffs, right_coeffs, oh, ow
				)
				# print("After Update", left_coeffs, right_coeffs)
				if left_coeffs is not None and right_coeffs is not None:
					tcte = 0
					lftx = eval_curve(left_coeffs, tcte)
					rghtx = eval_curve(right_coeffs, tcte)
					lftA1 = left_coeffs[0][1]
					lftB1 = left_coeffs[1][1]
					rghtA1 = right_coeffs[0][1]
					rghtB1 = right_coeffs[1][1]
					debug = pp.control(lftx[0], rghtx[0], lftA1, lftB1, rghtA1, rghtB1)
			...
			cv2.imwrite("bb.jpeg", left_panel)
			right_panel = _build_right_panel(
				left_pts, right_pts, left_coeffs, right_coeffs, bev_frame, lka_bev,
				virtual_side,
				left_det_pts=left_det_pts,
				right_det_pts=right_det_pts,
			)
			cv2.imwrite("tests.jpeg", right_panel)
