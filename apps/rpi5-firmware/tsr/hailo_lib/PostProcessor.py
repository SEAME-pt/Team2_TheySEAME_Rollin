...
class PostProcessor:
    def __init__(self, input_size=(640, 640), strides=(8, 16, 32), labels=None):
    ...
    def _split_box_cls(self, tensors):
        # Pick cls tensor as the one with smallest channel count
        tensors = sorted(tensors, key=lambda item: item[1].shape[-1])
        cls_name, cls_tensor = tensors[0]
        box_name, box_tensor = tensors[-1]
    ...
    def draw_detections(self, frame, detections):
        ...
        for box, score, cls_id in zip(boxes, scores, classes):
            ...
            if 0 <= int(cls_id) < len(self.labels):
