...
- **Uses** CAN ID `0x4D` (77 decimal) for battery messages
...
- **Standard 11-bit Identifier**: `0x4D` (77 in decimal)
- **Data Length**: 1 byte
...
- `0x4D` - Battery data (our message)
- `0x100` - Control commands (throttle/steering)
...
uint16_t can_id = 0x4d;  // Current ID (77 decimal)
// Change to different ID if needed, e.g., 0x4E
```
