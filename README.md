# They SEA:ME Rollin

SEA:ME 2025 Cohort - Team 02


## 📘 Overview

Intended for learning purposes, this project aims to deliver a small autonomous Raspberry Pi vehicle with autonomous navigation capabilities and AI decision-making, with a focus on safety and  reliability.


## 🚀 Current Sprint

| Field | Details |
|-------|----------|
| **Sprint Number** | Sprint 4 |
| **Sprint Goal** | Lay the groundwork to enable the first **Script-Driven Integration Test (SDFT)** of the piRacer car operating using scripted driving data, without remote control or ADAS. |
| **Sprint Duration** | Nov 24 – Dec 5, 2025 |
| **Focus Areas** | AGL, ThreadX, CAN, Open Fast Trace |


## 📅 Scrum Team

| Role | Name | Responsibilities |
|------|------|------------------|
| **Product Owner** | João | Manages backlog, defines priorities & value |
| **Scrum Master** | Marco | Facilitates Scrum process, removes blockers |
| **Developers** | Rui, Hugo, Bruno | Build, test, and deliver product increments |


## 🧩 Project Structure

```
├── docs/ # Documentation
├── libs/ # Shared Libraries between apps
├── apps/ # All developed apps
├── tests/ # E2E tests
├── scripts/ # Utility scripts (build, deploy, etc.)
└── README.md # This file
```

## 🏗️ Setup & Installation

**Requirements:**


**Steps:**


## 🔄 Branching & Workflow

This project branching strategy is explained in this [doc](https://github.com/SEAME-pt/Team2_TheySEAME_Rollin/blob/develop/docs/GitFlow.md)

## 📈 Product Backlog & Tracking

🗂️ Backlog Management: https://github.com/orgs/SEAME-pt/projects/82/views/15

🔍 Sprint Board: https://github.com/orgs/SEAME-pt/projects/82/views/5?filterQuery=+-sprint%3A%3C%40current

🧭 Roadmap:https://github.com/orgs/SEAME-pt/projects/82/views/6

## 🤝 Working Agreements
1. Daily Stand-up at 11 AM (or alternatively at 2.30 PM).
2. Two code reviews required before merging.
3. Definition of Done (DoD) includes testing, documentation, and code review by 2 peers.
4. Retrospective held at the end of each sprint.

