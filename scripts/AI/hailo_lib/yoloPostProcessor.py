...
try:
	...
	def cnms(preds, iou_thres):
		"""Fallback NMS returning kept indices when cython_nms is unavailable."""
...
try:
	...
	try:
		...
		def id_to_color(idx):
			# Deterministic fallback color mapping when toolbox is unavailable.
			rng = np.random.default_rng(int(idx) + 1337)
			return rng.integers(low=0, high=255, size=3, dtype=np.uint8)
try:
...
def _yolov8_decoding(raw_boxes, strides, image_dims, reg_max):
	...
	for box_distribute, stride in zip(raw_boxes, strides):
		box_distribute = box_distribute.astype(np.float32, copy=False)
		# create grid
...
def yolov8_seg_postprocess(endnodes, device_pre_post_layers=None, **kwargs):
	...
	scores = np.concatenate(scores, axis=1).astype(np.float32, copy=False)
	scores = expit(scores)
	outputs = []
...
def decode_and_postprocess(raw_detections, config_data, arch_key):
	...
	# for name, tensor in raw_detections.items():
	...
	# for i, tensor in enumerate(endnodes):
		# print(f"endnodes[{i}]: shape={tensor.shape}, name={list(raw_detections.keys())[list(raw_detections.values()).index(tensor)]}")
...
def draw_detections_no_nms(detections, img, config_data, labels, arch, tracker=None):
	...
	# print(f"Number of detections: {len(detections['detection_boxes'])}")
