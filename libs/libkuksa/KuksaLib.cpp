...
void kuksaLib::setCcActive(bool active) {
    ccActive.store(active);
}

void kuksaLib::setCcTargetSpeed(float speedValue) {
    ccTargetSpeed.store(speedValue);
}
    ...
    while (stream->Read(&resp)) {
        ...
        for (auto it = entries.begin(); it != entries.end(); ++it) {
            ...
            if (path == "Vehicle.Control.Throttle.Value") {
                ...
                bool active = false;
                if (valueToType(datapoint.value(), active)) {
                    setCcActive(active);
                }
            }
            else if (path == "Vehicle.ADAS.CruiseControl.TargetSpeed") {
                setCcTargetSpeed(value);
            }
        }
    }
